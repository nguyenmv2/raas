package com.github.nguyenmv2.raas.http

import com.github.nguyenmv2.raas.Fail
...
class EndpointsToRoutes(http: Http, apiContextPath: String) {
  ...
  def toDocsRoutes(es: ServerEndpoints): HttpRoutes[Task] = {
    val openapi = es.toList.toOpenAPI("Raas", "1.0").servers(List(Server(s"$apiContextPath", None)))
